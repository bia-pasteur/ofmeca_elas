...
def save_of_strain_traction(
    ...
    if not isinstance(next(iter(results.keys())), int):
        results = {0: results}
    ...
    for j, flow in enumerate(flows):
        ...
        ax.imshow(images[implot][0], cmap="gray", zorder=0)
...
def plot_reg(
    rmse_mean_reg: pd.DataFrame, factors_for_reg: list[float], results_dir: Path
):
    """
    ...
    axes = axes.flatten()
    titles = [
        "Deformation (regularization study)",
        "Traction (regularization study)",
        "Deformation (noise study)",
        "Traction (noise study)",
    ]
...
def plot_mean_error_noise(
    rmse_mean_noise: pd.DataFrame, stds: list[float], results_dir: Path
):
    """
    Saves a graph of the RMSE of the deformation and the traction for different
...
def plot_noise_reg(
    ...
    methods_reg = list(rmse_mean_reg["deformation"].keys())
    methods_plot_reg = [(m, name_map.get(m, m)) for m in methods_reg]
    ...
    for i, (ax, key) in enumerate(zip(axes[:2], keys_reg)):
        for j, (method_key, method_label) in enumerate(methods_plot_reg):
            ax.plot(
                factors_for_reg,
                rmse_mean_reg[key][method_key],
                linestyle=line_styles[j % len(line_styles)],
    ...
    for i, (ax, key) in enumerate(zip(axes[2:], keys_noise)):
        for j, (method_key, method_label) in enumerate(methods_plot_noise):
            ax.plot(
                stds,
                rmse_mean_noise[key][method_key],
                linestyle=line_styles[j % len(line_styles)],
    ...
    fig.legend(
        handles, labels, fontsize=25, loc="upper center", ncol=len(methods_plot_noise)
    )
...
def plot_cell_positions(
    ...
    f = lambda v: plt.scatter(
        [], [], marker=v[1], facecolor=v[0], s=v[2], linewidths=1, edgecolors="black"
    )
...
def plot_pos_dis_strain_trac_micro_image(
    ...
    Args:
        ...
        vmaxstrain (float): Maximum strain value for color normalization in plots.
        scale_flow (float): Scaling factor for quiver visualization of the displacements vectors.
        step_flow (int): Sampling step for displaying displacement vectors.
        scale_traction (float): Scaling factor for quiver visualization of the traction forces.
        step_traction (int): Sampling step for displaying traction vectors.
        show (bool, optional): If True, displays the generated figure interactively.
        vmin (float, optional): The value in the image to be mapped at 0.
        vmax (float, optional): The value in the image to be mapped at 1.
        alpha (float): Darkness control of the background. Must be between 0 and 1.
    """
    ...
    handles = [f(v) for _, v in channels.items()]
    labels = [v[3] for _, v in channels.items()]
    ...
    gs = gridspec.GridSpec(
        ...
        wspace=0.17,
        hspace=0.17,
        width_ratios=[2.2] + [1] * num_methods,
    )
    ...
    for j, flow in enumerate(flows):
        ...
        ax.imshow(image[0], cmap="gray", zorder=0, alpha=0.5)
        ...
        quiv_flow = ax.quiver(
            ...
            norm,
            cmap="afmhot",
            clim=(0, norm.max() if norm.max() > 0 else 1),
            angles="xy",
            scale_units="xy",
            scale=scale_flow,
            zorder=1,
        ...
        ax.axis("off")
        ax.set_title(method_names[j], fontsize=13)
...
def plot_field(image: np.ndarray, flow: np.ndarray, step: int, scale: float):
    ...
    cbar = fig.colorbar(quiv, ax=ax, orientation="vertical", fraction=0.05, pad=0.03)
    cbar.ax.tick_params(labelsize=10)
